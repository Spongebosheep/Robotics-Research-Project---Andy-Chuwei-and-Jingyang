...
def get_search_grid_cache(geo_model, L=1.0, search_grid_n=15):
    ...
    cache_key = (float(L), search_grid_n)
    ...
    all_d_grid = _bilinear_interp_stacked_batch(
        geo_model["distance_fields"], geo_model["xs"], geo_model["ys"], pts
    )

    payload = {
        "pts": pts,
        "all_d_grid": all_d_grid,
    }
...
def precompute_static_tdoa_templates(
    amplitude_bank,
    snr_bank,
    detected_mask_bank,
    ref_strategy="max_snr",
    ...
    M = detected_mask_bank.shape[0]
    templates = [None] * M
    static_ref = ref_strategy in {"max_snr", "max_amplitude"}

    if not static_ref:
        return templates

    for event_id in range(M):
        detected_mask = detected_mask_bank[event_id]
        detected_idx = np.flatnonzero(detected_mask)
        n_detect = detected_idx.size
        if n_detect < min_detect:
        ...
        ref = choose_reference_node(
            t=np.zeros(detected_mask.shape[0], dtype=float),
            detected_idx=detected_idx,
            strategy=ref_strategy,
            amplitude=amplitude_bank[event_id],
            snr=snr_bank[event_id],
...
def evaluate_precomputed_bank_at_noise_level(
    ...
    for event_id, q_true in enumerate(q_true_bank):
        ...
        template = None if static_tdoa_templates is None else static_tdoa_templates[event_id]
        if template is not None:
            ref = template["ref"]
            use_idx = template["use_idx"]
            obs = {
                "detected_idx": template["detected_idx"],
                "n_detect": template["n_detect"],
                "ref": ref,
                "use_idx": use_idx,
                "dt_obs": t[use_idx] - t[ref],
    ...
    for seed in range(n_seeds):
        ...
        for terrain in terrains:
            ...
            for layout in layouts:
                ...
                static_tdoa_templates = precompute_static_tdoa_templates(
                    amplitude_bank=amplitude_bank,
                    snr_bank=snr_bank,
                    detected_mask_bank=detected_mask_bank,
                    ref_strategy=ref_strategy,
                ...
                for sigma_t in sigma_t_levels:
                    ...
                    eval_stats = evaluate_precomputed_bank_at_noise_level(
                        ...
                        static_tdoa_templates=static_tdoa_templates,
                    )
