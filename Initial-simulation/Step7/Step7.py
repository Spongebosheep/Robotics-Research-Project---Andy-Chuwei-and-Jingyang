...
# =========================
# Step 7 script
# Dynamic reference node for TDoA
#
# Forward model remains arrival-time only:
#   t_i = t0 + d_path(r_i, s) / v_eff + eps_i
#   eps_i ~ N(0, sigma_t^2)
#
# New in Step 7:
# - do NOT fix node 1 as the TDoA reference
# - build the TDoA observation from a detected-node set D
# - choose the reference dynamically from D
#     * earliest detected arrival
#     * largest amplitude      (hook for future use)
#     * highest SNR            (hook for future use)
# - estimate q=(x,y) on the terrain surface via
#
#   q_hat = argmin_q sum_{i in D\{r}} w_i [dt_i - (d_i(q)-d_r(q))/v_eff]^2
#
# Notes for this Step 7 version:
# - detection threshold / amplitude model is NOT added yet
# - therefore, by default, all sensors are treated as detected
# - the code is written so Step 8 can later replace detected_mask
#   with a true threshold-based detecting set
# =========================
...
# -------------------------
# Step 7 dynamic-reference settings
# -------------------------
REF_STRATEGY = "earliest_time"   # "earliest_time", "max_amplitude", "max_snr"
WEIGHT_STRATEGY = "uniform"      # "uniform", "amplitude", "snr"
MIN_DETECT = 4                    # useful once detection threshold is added
...
# =========================================================
# Step 7: arrival-time forward model (absolute times only)
# =========================================================
def simulate_arrival_times_versionB(
    ...
    return t, s_true, d_path


# =========================================================
# Step 7: dynamic reference-node selection on detecting set D
# =========================================================
...
def choose_reference_node(
    t,
    detected_idx,
    strategy="earliest_time",
    amplitude=None,
    snr=None,
):
...
def make_tdoa_weights(use_idx, strategy="uniform", amplitude=None, snr=None):
...
def estimate_surface_source_xy_versionB_step7(
    obs,
    geo_model,
    L=1.0,
    v_eff=1.0,
...
def plot_terrain_examples(L=1.0):
    ...
    plt.suptitle("Step 7 terrain models")
    plt.show()
...
def plot_deployment_maps(L=1.0, seed=0):
    ...
    fig.suptitle("Step 7 deployment visualization", fontsize=15)
    plt.show()
...
def plot_step7_summary(summary_df):
    terrains = ["flat", "hill", "rough"]
...
# =========================================================
def run_step7_dynamic_reference(
    L=1.0,
    M=150,
    n_seeds=3,
    sigma_t=0.002,
    ...
    for terrain in terrains:
        for seed in range(n_seeds):
            ...
            # Shared event set and timing noise across layouts for fairness
            ...
            for layout in layouts:
                ...
                for event_id, q_true in enumerate(source_params_xy):
                    ...
                    # Step 7: no threshold yet -> all sensors are treated as detecting
                    detected_mask = np.ones(n_sensors, dtype=bool)
                    amplitude = None
                    snr = None
                    ...
                    if not usable or obs is None:
                        n_failed_observation += 1
                        continue

                    detected_counts.append(obs["n_detect"])
                    ref_nodes.append(obs["ref"])

                    q_hat, ok, fit_resnorm = estimate_surface_source_xy_versionB_step7(
                        obs=obs,
                        geo_model=geo_model,
                        L=L,
                        v_eff=v_eff,
                ...
                solver_valid_rate = n_solver_valid / M
                usable_event_rate = (M - n_failed_observation) / M

                ref_counts = np.bincount(ref_nodes, minlength=n_sensors) if ref_nodes else np.zeros(n_sensors, dtype=int)
                ...
                rows.append({
                    ...
                    "solver_valid_rate": float(solver_valid_rate),
                    "failed_observation": int(n_failed_observation),
                    "failed_localization": int(n_failed_localization),
                    "mean_detected_count": float(np.mean(detected_counts)) if detected_counts else np.nan,
    ...
    summary = (
        ...
        .agg(
            ...
            solver_valid_rate=("solver_valid_rate", "mean"),
            mean_detected_count=("mean_detected_count", "mean"),
...
if __name__ == "__main__":
    raw_df, summary_df = run_step7_dynamic_reference(
        L=L,
        M=N_EVENTS,
        n_seeds=N_SEEDS,
        sigma_t=SIGMA_T,
    ...
    print("\n=== Step 7 summary (dynamic reference node on detecting set, geodesic TDoA) ===")
    print(summary_df.to_string(index=False))

    if SAVE_CSV:
        raw_df.to_csv("step7_dynamic_reference_raw_results.csv", index=False)
        summary_df.to_csv("step7_dynamic_reference_summary.csv", index=False)
        print("\nSaved: step7_dynamic_reference_raw_results.csv, step7_dynamic_reference_summary.csv")
