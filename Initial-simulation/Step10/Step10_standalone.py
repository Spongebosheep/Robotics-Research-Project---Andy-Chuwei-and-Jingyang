...
def build_shared_event_bank(
    ...
    timing_noise_bank = rng.normal(0.0, sigma_t, size=(M, n_sensors))
...
if __name__ == "__main__":
    ...
    raw_df, step9_summary_df, fairness_df = run_step9_monte_carlo(
        L=L,
        M=N_EVENTS,
        n_seeds=N_SEEDS,
        sigma_t=SIGMA_T,
    ...
    step10_raw_df = add_step10_metrics(raw_df, w_edge=DGEOM_W_EDGE, w_height=DGEOM_W_HEIGHT)
    step10_summary_df, step10_main_table = build_step10_summary(step10_raw_df)

    pd.set_option("display.width", 320)
    pd.set_option("display.max_columns", 200)

    print("\n=== Step 10 main metrics table ===")
    print(step10_main_table.to_string(index=False))

    print("\n=== Step 10 extended summary ===")
    print(step10_summary_df.to_string(index=False))
    ...
    if SAVE_STEP10_CSV:
        step10_raw_df.to_csv("step10_metrics_raw_results.csv", index=False)
        step10_summary_df.to_csv("step10_metrics_summary.csv", index=False)
        step10_main_table.to_csv("step10_main_metrics_table.csv", index=False)
        fairness_df.to_csv("step10_fairness_check.csv", index=False)
        print("\nSaved: step10_metrics_raw_results.csv, step10_metrics_summary.csv, step10_main_metrics_table.csv, step10_fairness_check.csv")
    ...
    if PLOT_SUMMARY:
        plot_step9_summary(step9_summary_df)

    if PLOT_STEP10_CORE:
        plot_step10_core_metrics(step10_summary_df)

    if PLOT_STEP10_GEOMETRY:
        plot_step10_geometry_components(step10_summary_df)
