            ...
            for layout in layouts:
                ...
                for event_id, q_true in enumerate(source_params_xy):
                    event = simulate_event_step9(
                        ...
                        local_coupling_mode=local_coupling_mode,
                        eta_log_std=eta_log_std,
                        eta_override=shared_eta[event_id],
                        L=L,
                    )
...
if __name__ == "__main__":
    raw_df, summary_df, fairness_df = run_step9_monte_carlo(
        L=L,
        M=N_EVENTS,
        n_seeds=N_SEEDS,
        sigma_t=SIGMA_T,
    ...
    print("\n=== Step 9 summary (fixed Monte Carlo design + fair shared event banks) ===")
    print(summary_df.to_string(index=False))

    print("\n=== Step 9 fairness check (each seed/terrain should see 3 layouts and 1 shared event bank) ===")
    print(fairness_df.to_string(index=False))

    if SAVE_CSV:
        raw_df.to_csv("step9_monte_carlo_raw_results.csv", index=False)
        summary_df.to_csv("step9_monte_carlo_summary.csv", index=False)
        fairness_df.to_csv("step9_monte_carlo_fairness_check.csv", index=False)
        print("\nSaved: step9_monte_carlo_raw_results.csv, step9_monte_carlo_summary.csv, step9_monte_carlo_fairness_check.csv")
